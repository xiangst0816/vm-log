...
class Log {
  constructor (Vue) {
    ...
    this._logPageInstance = {} // log页面的实例
    this.init()
  }
   ...
   * */
  init () {
    if (!this._isLogPageInit && logComponent && this._vue) {
      let LogConstructor = this._vue.extend(logComponent)
   ...
   * */
  _storeRecord (record) {
    if (this._logPageInstance) {
      let history = this._logPageInstance.recordList
      if (history) {
        let _len = history.length
        if (_len > 0) {
          let _lastRecord = history.pop()
